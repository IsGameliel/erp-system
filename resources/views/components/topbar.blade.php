@php
    ...
    $ownerRouteBase = request()->routeIs('owner.*');
...
<div x-data="{ mobileMenuOpen: false }" @keydown.escape.window="mobileMenuOpen = false" class="lg:contents">
    <div class="relative border-b border-slate-200/80 bg-white/90 backdrop-blur">
        <div class="flex items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
            <div class="flex items-center gap-3">
                <button
                    type="button"
                    class="inline-flex h-11 w-11 items-center justify-center rounded-full border border-slate-200 text-slate-700 transition hover:border-cyan-200 hover:text-cyan-700 md:hidden"
                ...
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.25em] text-cyan-700">{{ $applicationBrandName }}</p>
                    <p class="mt-1 text-sm text-slate-500">{{ $isOwner ? 'Owner platform controls and subscription monitoring.' : 'Operational dashboard for sales and procurement.' }}</p>
                </div>
            </div>
    ...
    <aside
        ...
        style="display: none;"
        class="fixed inset-y-0 left-0 z-50 w-[280px] max-w-[85vw] overflow-y-auto border-r border-slate-200/80 bg-[linear-gradient(180deg,#0f172a_0%,#111827_18%,#1e293b_100%)] text-white shadow-2xl lg:hidden"
    >

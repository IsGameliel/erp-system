<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
            <div>
                <h2 class="text-3xl font-semibold tracking-tight text-slate-950">{{ $salesOrder->order_number }}</h2>
                <p class="mt-2 text-sm text-slate-500">Sales order details and item lines.</p>
            </div>
            <div class="flex gap-3">
                <a href="{{ route('sales-orders.receipt', $salesOrder) }}" class="btn-secondary">Print receipt</a>
                <a href="{{ route('sales-orders.edit', $salesOrder) }}" class="btn-primary">Edit order</a>
            </div>
        </div>
    </x-slot>
    ...
    <div class="grid gap-6 xl:grid-cols-3">
        <div class="page-panel xl:col-span-2">
            ...
            <dl class="mt-4 grid gap-4 md:grid-cols-2 text-sm">
                <div><dt class="text-slate-500">Customer</dt><dd class="mt-1 text-slate-900">{{ $salesOrder->customer?->full_name }}</dd></div>
                <div><dt class="text-slate-500">Store</dt><dd class="mt-1 text-slate-900">{{ $salesOrder->store?->name ?? 'Unassigned' }}</dd></div>
                <div><dt class="text-slate-500">Sales officer</dt><dd class="mt-1 text-slate-900">{{ $salesOrder->user?->name }}</dd></div>
                <div><dt class="text-slate-500">Order date</dt><dd class="mt-1 text-slate-900">{{ optional($salesOrder->order_date)->format('M d, Y') }}</dd></div>
                <div><dt class="text-slate-500">Status</dt><dd class="mt-1"><x-status-badge :status="$salesOrder->status" /></dd></div>
                <div><dt class="text-slate-500">Payment status</dt><dd class="mt-1 text-slate-900">{{ ucfirst($salesOrder->payment_status) }}</dd></div>
                <div><dt class="text-slate-500">Payment method</dt><dd class="mt-1 text-slate-900">{{ $salesOrder->payment_method ? strtoupper($salesOrder->payment_method) : 'Credit / Pay later' }}</dd></div>
                <div class="md:col-span-2"><dt class="text-slate-500">Notes</dt><dd class="mt-1 text-slate-900">{{ $salesOrder->notes ?: 'No notes available.' }}</dd></div>
            </dl>
        </div>
    ...
    <div class="page-panel">
        ...
        <div class="mt-4 space-y-4">
            @forelse ($salesOrder->activityLogs as $activity)
